...
class InterpreterTest(unittest.TestCase):
	...
	def test_execute_the_command_when_match_found(self):
		...
		command_return_value = self.interpreter.eval(IRRELEVANT_KEY)
		
		assert_that(command.execute, called().with_args(tokens=[IRRELEVANT_KEY], interpreter=self.interpreter))

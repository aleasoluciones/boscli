...
with describe('Help'):
    ...
    with before.each:
        ...
        self.cmd1 = Command(['cmd', 'key1'], self.command_implementation.cmd1, help='help_cmd1', always=True)
    ...
    with context('when asking for help with empty line'):
        ...
        with describe('when inside a context'):
            ...
            with it('returns help for commands that are always active'):
                self.interpreter.push_context('irrelevant_context')
            ...
            with it('does not returns help for normal commands (command from main context without always flag)'):
                self.interpreter.push_context('irrelevant_context')
